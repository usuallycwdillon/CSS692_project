...
rGi.sub52     <- graph.data.frame(mm.sub5wc2, directed=TRUE)
rGi.sub55     <- graph.data.frame(mm.sub5wc5, directed=TRUE)
rGi.sub55C    <- graph.data.frame(mm.sub5w5c500, directed=TRUE)
...
# Plot the dendrigram and look for blocking 
dendrigram <- function (graph) {
...
dendrigram(rGi)
dendrigram(rGi.sub52)
dendrigram(rGi.sub55)
dendrigram(rGi.sub55C)
dendrigram(rGi.sub55C55C)
...
title(main="Diameter of the GitHub repository network",
      xlab="at least 5 watchers and contributors, \n no more than 500 watchers")
